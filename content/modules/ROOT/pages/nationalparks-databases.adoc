...
* *Show advanced Deployment options* をクリックします。
    ...
    - *+Add from ConfigMap or Secret* をクリックし、名前として `MONGO_INITDB_ROOT_USERNAME` と入力します。値には、リソースとして `mongodb-credentials` 、キーとして `admin-usr` を選択します。
    - 再度、*+Add from ConfigMap or Secret* をクリックし、名前として `MONGO_INITDB_ROOT_PASSWORD` と入力します。値には、リソースとして `MONGO_INITDB_ROOT_PASSWORD` 、キーとして `admin-pwd` を選択します。
...
NOTE: 商用環境では、コンテナのエントリポイントを変更したり、link:https://kubernetes.io/docs/concepts/workloads/pods/init-containers/[init container, window="_blank"] を使用してデータベース構成を管理したりできます。
