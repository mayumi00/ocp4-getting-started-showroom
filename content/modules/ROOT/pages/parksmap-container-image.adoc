...
NOTE: *Labels* はキーと値のペアです。同じ *Project* （ *ネームスペース/名前空間* とも呼ばれます）内の、*Selector* に一致するラベルを持つすべての *Pod* が、*Service* に関連付けられます。この動作を確認するには、以下のコマンドを実行してください。
...
なぜエンドポイントが1つしか表示されないのか疑問に思うかもしれません。これは、現在実行中の *Pod*が  1つしかないためです。次のラボでは、アプリケーションをスケーリングする方法を学習します。その時点で、*Service* に関連付けられた複数のエンドポイントが
表示されるようになります。
