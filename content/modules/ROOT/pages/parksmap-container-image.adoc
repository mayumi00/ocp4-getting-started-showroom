...
OpenShift（およびKubernetes）では、デプロイ可能な最小単位は *Pod* です。*Pod* とは、1つ以上のhttps://opencontainers.org/[OCI containers、window="_blank"]のグループであり、これらは一緒にデプロイされ、同じホスト上に配置されることが保証されています。
OpenShift公式ドキュメントより：
...
NOTE: 出力で正しい *Pod* 名を使用していることを確認してください。
